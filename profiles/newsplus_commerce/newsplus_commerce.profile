 ...
 */
function newsplus_commerce_install_tasks_alter(&$tasks, $install_state) {
	...
	$tasks['newsplus_commerce_content'] = array(
 ...
 */
function newsplus_commerce_form_install_configure_form_alter(&$form, $form_state) {
	$form['site_information']['site_name']['#default_value'] = 'News+';
    $form['#submit'][] = 'newsplus_commerce_install_configure_form_submit';
}

/**
 * Forms API submit for the site configuration form.
 */
function newsplus_commerce_install_configure_form_submit($form, &$form_state) {
	global $user;
	...
	theme_enable(array('newsplus', 'seven'));
	variable_set('theme_default', 'newsplus');
	variable_set('admin_theme', 'seven');
	variable_set('file_temporary_path', 'sites/default/files/tmp');
	
	drupal_set_message("News+ Commerce Easy Installation was a Success!");

}

function newsplus_commerce_content(&$install_state) {
	$sql_file=dirname(__FILE__).'/newsplus_commerce.sql';
	$count=newsplus_commerce_import_sql($sql_file);
}

function newsplus_commerce_import_sql($filename){
