 ...
 */
function newsplus_install_tasks_alter(&$tasks, $install_state) {
 ...
 */
function newsplus_form_install_configure_form_alter(&$form, $form_state) {
	$form['site_information']['site_name']['#default_value'] = 'News+';
    $form['#submit'][] = 'newsplus_install_configure_form_submit';
}

/**
 * Forms API submit for the site configuration form.
 */
function newsplus_install_configure_form_submit($form, &$form_state) {
	global $user;
	...
	drupal_set_message("News+ Easy Installation was a Success!");

}

function newsplus_content(&$install_state) {
	$sql_file=dirname(__FILE__).'/newsplus.sql';
	$count=newsplus_import_sql($sql_file);
}

function newsplus_import_sql($filename){
