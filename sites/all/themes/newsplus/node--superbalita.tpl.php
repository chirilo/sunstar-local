  ...
  <div id="printArea" class="node-content-wrapper clearfix">
      ...
      <header>
        ...
        <?php print render($title_suffix); ?>
          ...
          <div class="node-info">
            ...
            <!-- display author in opinyon only -->            
            <div class="node-info-item">
                <?php if ($content['sunstar_with_author_image']['#value'] && $content['sunstar_term_name_section']['#value'] == "Opinyon") { ?>
                <img src="<?php print $content['sunstar_author_image_url']['#value']; ?>" alt="<?php print $content['sunstar_author_name']['#value']; ?>" />
                <?php } ?>               
                <i class="fa fa-user"></i> <?php print t('Ni ');?><?php print render($content['field_author']) . render($content['field_column']) ;  ?>
            </div>
            <?php } ?>
            <!-- display author in opinyon only -->
          </div>
    ...
    <?php 
      ...
      */
      ...
      if($used_loc=='superbalita-cebu') {
        $tmp = split('-', $used_loc);        
        $format_loc = 'Superbalita'.'-'.ucfirst($tmp[1]);
        echo "<p id='publication'>Latest issues of <strong>SunStar ".$format_loc."</strong> also available on your mobile phones, laptops, and tablets. Subscribe to our digital editions at <a href='http://epaper.sunstar.com.ph/epaper/viewer.aspx' target='_blank'>epaper.sunstar.com.ph</a> and get a free seven-day trial.</p>";
      }
    ?>
